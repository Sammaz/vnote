...
impl Database {
    ...
    fn init_tables(&self) -> SqliteResult<()> {
        ...
        // Migration: Add panoramic_blueprint column if not exists
        let has_panoramic_blueprint: bool = conn
            .prepare("SELECT COUNT(*) FROM pragma_table_info('notes') WHERE name='panoramic_blueprint'")?
            .query_row([], |row| row.get::<_, i64>(0))
            .map(|count| count > 0)
            .unwrap_or(false);

        if !has_panoramic_blueprint {
            conn.execute(
                "ALTER TABLE notes ADD COLUMN panoramic_blueprint TEXT",
                [],
            )?;
        }

        // Migration: Add quick_notes column if not exists
        let has_quick_notes: bool = conn
            .prepare("SELECT COUNT(*) FROM pragma_table_info('notes') WHERE name='quick_notes'")?
            .query_row([], |row| row.get::<_, i64>(0))
            .map(|count| count > 0)
            .unwrap_or(false);
        ...
        if !has_quick_notes {
            conn.execute(
                "ALTER TABLE notes ADD COLUMN quick_notes TEXT",
                [],
            )?;
        }

        // Migration: Add quick_notes_mindmap column if not exists
        let has_quick_notes_mindmap: bool = conn
            .prepare("SELECT COUNT(*) FROM pragma_table_info('notes') WHERE name='quick_notes_mindmap'")?
            .query_row([], |row| row.get::<_, i64>(0))
            .map(|count| count > 0)
            .unwrap_or(false);

        if !has_quick_notes_mindmap {
            conn.execute(
        ...
        // Migration: Add quick_notes_canvas column if not exists
        let has_quick_notes_canvas: bool = conn
            .prepare("SELECT COUNT(*) FROM pragma_table_info('notes') WHERE name='quick_notes_canvas'")?
            .query_row([], |row| row.get::<_, i64>(0))
            .map(|count| count > 0)
            .unwrap_or(false);

        if !has_quick_notes_canvas {
            conn.execute(
