...
pub struct Note {
    ...
    pub visual_summary_mindmap: Option<String>,
    ...
    pub fn get_all_notes(&self) -> SqliteResult<Vec<Note>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, title, video_path, subtitle_path, model_id, init_status, full_summary, detailed_reading,
                    highlights, visual_summary, visual_summary_mindmap, custom_summary, ai_note_markdown, ai_note_original_markdown, ai_note_meta,
        ...
        let notes = stmt.query_map([], |row| {
            Ok(Note {
                ...
                visual_summary_mindmap: row.get(10)?,
                custom_summary: row.get(11)?,
                ai_note_markdown: row.get(12)?,
                ai_note_original_markdown: row.get(13)?,
                ai_note_meta: row.get(14)?,
                flashcards: row.get(15)?,
                panoramic_blueprint: row.get(16)?,
                quick_notes: row.get(17)?,
                quick_notes_mindmap: row.get(18)?,
                quick_notes_canvas: row.get(19)?,
                suggested_questions: row.get(20)?,
                last_playback_position: row.get(21)?,
    ...
    pub fn update_note(&self, note: &Note) -> SqliteResult<()> {
        ...
        conn.execute(
            "UPDATE notes SET
                ...
                visual_summary = ?9, visual_summary_mindmap = ?10, custom_summary = ?11, ai_note_markdown = ?12,
                ai_note_original_markdown = ?13, ai_note_meta = ?14, flashcards = ?15, panoramic_blueprint = ?16,
                quick_notes = ?17, quick_notes_mindmap = ?18, quick_notes_canvas = ?19, suggested_questions = ?20,
                last_playback_position = ?21,
            ...
            rusqlite::params![
                ...
                &note.visual_summary, &note.visual_summary_mindmap, &note.custom_summary, &note.ai_note_markdown,
                &note.ai_note_original_markdown, &note.ai_note_meta, &note.flashcards, &note.panoramic_blueprint,
                &note.quick_notes, &note.quick_notes_mindmap, &note.quick_notes_canvas, &note.suggested_questions,
